...
const mcp = async (method, params, id = 1) => {
  const r = await fetch(BASE + '/mcp', { method: 'POST', headers: { 'Content-Type': 'application/json', Accept: 'application/json, text/event-stream', Authorization: 'Bearer ' + token }, body: JSON.stringify({ jsonrpc: '2.0', id, method, params }) });
...
await t('tools/list elenca gli strumenti', async () => { const r = await mcp('tools/list', {}, 2); const names = r.json.result.tools.map((x) => x.name).sort(); assert.deepStrictEqual(names, ['cerca_documenti', 'chiedi_archivio', 'correggi_documento', 'dettaglio_documento', 'documenti_recenti', 'link_documento']); });
