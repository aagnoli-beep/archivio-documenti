...
t('file: contenuto base64 di un documento indicizzato, rifiuto per file estranei', () => { const r = call({ action: 'file', token: 'good', id: f1.getId() }); assert.strictEqual(r.ok, true); assert.strictEqual(Buffer.from(r.data.base64, 'base64').length, pdfBytes.length); assert.strictEqual(r.data.mime, 'application/pdf'); assert.strictEqual(r.data.more, false); assert.strictEqual(r.data.size, pdfBytes.length); const other = G.__mkfile(root, 'segreto.pdf', pdfBytes, 'application/pdf'); assert.strictEqual(call({ action: 'file', token: 'good', id: other.getId() }).code, 'not_found'); });
t('file a pezzi: offset/length e flag more', () => { const a = call({ action: 'file', token: 'good', id: f1.getId(), length: 5000 }).data; assert.strictEqual(a.length, 5000); assert.strictEqual(a.more, true); const b = call({ action: 'file', token: 'good', id: f1.getId(), offset: 5000, length: 5000 }).data; const c = call({ action: 'file', token: 'good', id: f1.getId(), offset: 10000, length: 100000 }).data; assert.strictEqual(c.more, false); const all = Buffer.concat([a, b, c].map(x => Buffer.from(x.base64, 'base64'))); assert.strictEqual(all.length, pdfBytes.length); assert.ok(all.equals(Buffer.from(pdfBytes))); });
